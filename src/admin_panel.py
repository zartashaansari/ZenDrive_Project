...
class AdminDashboard:
    ...
    def draw(self):
        ...
        self.user_label = ctk.CTkLabel(header, text=f"Active Users: __{u}__", font=("Roboto", 14, "bold"))
        ...
        # 3. Recent Hazards Feed (Right)
        self.recent_bar = ctk.CTkFrame(middle_container, width=220, corner_radius=10, border_width=2, border_color=self.main_color)
        self.recent_bar.pack(side="right", fill="y")
        ctk.CTkLabel(self.recent_bar, text="Recent Hazards", font=("Roboto", 16, "bold"), text_color=self.main_color).pack(pady=15)
        
        # --- Sidebar Navigation Buttons ---
        ctk.CTkButton(nav_bar, text="🏠 Home", fg_color="transparent", command=self.show_home_performance).pack(fill="x", pady=10, padx=5)
        ctk.CTkButton(nav_bar, text="👥 User Management", fg_color="transparent", command=self.show_user_management).pack(fill="x", pady=10, padx=5)
        ctk.CTkButton(nav_bar, text="📋 Hazard Logs", fg_color="transparent", command=self.show_hazard_logs).pack(fill="x", pady=10, padx=5)
        ctk.CTkButton(nav_bar, text="📊 Visibility Analytics", fg_color="transparent", command=self.show_visibility_analytics).pack(fill="x", pady=10, padx=5)
        ctk.CTkButton(nav_bar, text="Generate CSV", fg_color="#1E8E5E", hover_color="#156e48", command=self.export_hazard_csv).pack(side="bottom", pady=20, padx=10, fill="x")
        ...
        # --- Initialization & Heartbeat Loops ---
    ...
    def refresh_stats(self):
        """Syncs the top header cards with CockroachDB stats every second."""
        try:
            if hasattr(self, 'user_label') and self.user_label.winfo_exists():
                u, h, v = self.db.get_admin_stats()
                self.user_label.configure(text=f"Active Users: __{u}__")
    ...
    def refresh_recent_hazards(self):
        """Syncs the right sidebar feed with the latest 5 cloud hazard events."""
        try:
            if hasattr(self, 'recent_bar') and self.recent_bar.winfo_exists():
                ...
                for h_type, h_time, h_user in recent_data:
                    # Robust timestamp formatting for cloud/local datetime objects
                    time_only = str(h_time).split(" ")[1][:5] if " " in str(h_time) else str(h_time)[:5]
                    lbl_text = f"{h_user}\n{h_type} ({time_only})"
                    ctk.CTkLabel(self.recent_bar, text=lbl_text, font=("Roboto", 11), pady=10).pack()
    ...
    def show_user_management(self):
        self.clear_view()
        ctk.CTkLabel(self.view_area, text="REGISTERED SYSTEM USERS", font=("Roboto", 20, "bold"), text_color=self.main_color).pack(pady=10)
        ...
        for user in users:
            ...
            for col, val in enumerate(user):
                ctk.CTkLabel(row, text=str(val)[:20], width=140).grid(row=0, column=col)

    def show_home_performance(self):
        """Displays a Bar Chart using CTkFrames based on cloud hazard data."""
        self.clear_view()
        ctk.CTkLabel(self.view_area, text="DAILY HAZARD DETECTION PERFORMANCE", font=("Roboto", 20, "bold"), text_color=self.main_color).pack(pady=20)
        ...
        for date, count in daily_data:
            ...
            ctk.CTkFrame(bar_container, width=40, height=bar_height, fg_color=self.main_color).pack(side="bottom")
            ctk.CTkLabel(bar_container, text=str(count), font=("Roboto", 10)).pack(side="bottom")
            ctk.CTkLabel(bar_container, text=str(date)[-5:], font=("Roboto", 10)).pack(side="bottom")
    ...
    def show_hazard_logs(self):
        ...
        for row in logs:
            ...
            # Log Map: 5=User, 0=Type, 1=Time, 2=Conf
            display_data = [row[5], row[0], str(row[1])[11:19], f"{row[2]:.2f}"]
    ...
    def show_visibility_analytics(self):
        """Displays a manual Line Chart on a Canvas as per the original UI design."""
        self.clear_view()
        ctk.CTkLabel(self.view_area, text="ROAD VISIBILITY TREND (LAST 7 DAYS)", font=("Roboto", 20, "bold"), text_color=self.main_color).pack(pady=20)
        ...
        canvas = ctk.CTkCanvas(self.view_area, width=600, height=300, bg="#1a1a1a", highlightthickness=0)
        canvas.pack(pady=10)
        ...
        for i in range(len(points) - 1):
            canvas.create_line(points[i][0], points[i][1], points[i+1][0], points[i+1][1], fill="#3498db", width=3, smooth=True)
        for i, (x, y) in enumerate(points):
            canvas.create_oval(x-4, y-4, x+4, y+4, fill="#3498db", outline="white")
            canvas.create_text(x, 300 - 20, text=dates[i], fill="white", font=("Roboto", 10))
            canvas.create_text(x, y - 15, text=f"{vis_scores[i]}%", fill="white", font=("Roboto", 9))

    def export_hazard_csv(self):
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", initialfile=f"ZenDrive_Report_{datetime.date.today()}.csv")
